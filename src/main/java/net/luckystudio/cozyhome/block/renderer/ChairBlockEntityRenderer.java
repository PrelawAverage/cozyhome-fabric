...
public class ChairBlockEntityRenderer implements BlockEntityRenderer<ChairBlockEntity> {
    ...
    private static final Map<ChairBlock.ChairType, Identifier> CHAIR_TEXTURES = Util.make(Maps.newHashMap(), map -> {
        ...
        map.put(ChairBlock.Type.TRIAL, Identifier.of(CozyHome.MOD_ID,"textures/block/chair/trial_chair.png"));
    });
    ...
    public ChairBlockEntityRenderer(BlockEntityRendererFactory.Context context) {
        // Use a custom model layer (make sure to register it in the client mod initializer)
        this.chair = context.getLayerModelPart(ChairModel.LAYER_LOCATION);
        this.cushion = context.getLayerModelPart(CushionModel.LAYER_LOCATION);
    }
    ...
    public void render(ChairBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(getRotationAngle(entity)));

        BlockState blockState = entity.getCachedState();
        ChairBlock.ChairType chairType = ((ChairBlock)blockState.getBlock()).getChairType();

        RenderLayer chairRenderLayer = getChairRenderLayer(chairType);
    ...
    private float getRotationAngle(ChairBlockEntity entity) {
        // Rotate the model based on the block's facing direction
        int rotation = entity.getCachedState().get(ChairBlock.ROTATION);
    ...
    public static RenderLayer getChairRenderLayer(ChairBlock.ChairType type) {
        Identifier identifier = CHAIR_TEXTURES.get(type);
