...
import net.luckystudio.cozyhome.block.entity.GrandfatherClockBlockEntity;
...
public class GrandfatherClockBlock extends BlockWithEntity implements Waterloggable{
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(ROTATION, TRIPLE_TALL_BLOCK);
    }
        ...
        switch (currentPart) { // Handle the logic based on which part of the block this is
            case TOP:
                if (direction == Direction.DOWN) { // Ensure the middle part is below and the block is placeable
                    ...
                    if (!belowState.isOf(this) || belowState.get(TRIPLE_TALL_BLOCK) != TripleTallBlock.MIDDLE) {
            ...
            case MIDDLE:
                if (direction == Direction.UP) { // Ensure the top part is above and the bottom part is below
                    ...
                    if (!aboveState.isOf(this) || aboveState.get(TRIPLE_TALL_BLOCK) != TripleTallBlock.TOP) {
                        return Blocks.AIR.getDefaultState();  // Break the block if the top part is missing
                    }
                } else if (direction == Direction.DOWN) {
                    BlockState belowState = world.getBlockState(pos.down());
                    if (!belowState.isOf(this) || belowState.get(TRIPLE_TALL_BLOCK) != TripleTallBlock.BOTTOM) {
                        return Blocks.AIR.getDefaultState();  // Break the block if the bottom part is missing
                    }
                }
                break;
            case BOTTOM:
